...
A fast, mutt-inspired TUI for the git operations you do most often. Inspired by `gitu`, but trimmed to the actions actually used and built around fast per-file/per-section diffing.

## Principles

- **Slim scope.** Only build what's needed for the daily stage → commit flow. Every additional feature must be motivated by a real missing-it moment, not speculation.
...
internal/git/            thin wrappers around the git CLI
  ...
  log.go                 `git log -n 50 --oneline --decorate=short`
...
- Tagged rows are visually marked (`*` prefix).
...
**Recent commits.** `git log -n 50 --oneline --decorate=short`. Show alongside any decorations (branch/tag refs). `l` unfolds a commit to show changed files with +/- line counts; `d` on a file runs `git show <sha> -- <file>`; `h` collapses.
...
Push / pull / fetch · branch create/switch/delete · stash apply/pop/drop · rebase · merge · conflict resolution UI · hunk-level staging (`git add -p`) · log browsing beyond last 50 · file-watcher live refresh · non-TUI subcommands (`gt status`, etc.) · search / filter · settings file · themes.

Each of these gets added only when it's actually missed in daily use.
